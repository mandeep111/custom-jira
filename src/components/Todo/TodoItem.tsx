...
import { update } from '../../services/State';
...
const Component: React.FunctionComponent<TodoItem> = ({ task, index, currentColumnName, moveCardHandler, setItems, fetchingData }) => {
    ...
    const changeItemColumn = (currentItem: { task: Task }, columnId: number) => {
        setItems((prevState) => {
            return prevState.map((e) => {
                try {
                    void update(currentItem.task.id, columnId, config);
                } catch (error) {
    ...
    return (
        <React.Fragment>
            ...
            >
                ...
                <div className="flex grid-cols-1  justify-items-start  whitespace-nowrap overflow-hidden" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                    {/* {isPopoverVisible && <PopoverAssignee />} */}
                    {task.assignees ? (
