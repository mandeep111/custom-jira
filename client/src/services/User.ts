...
export const save = async <T,>(data: T, config: HeaderConfig) => {
    try {
        const response: AxiosResponse<T, T> = await axios.post(API.USER, data, config);
...
export const getAll = async <T>(config: HeaderConfig) => {
    try {
        const response: AxiosResponse<T> = await axios.get(`${API.USER}/all`, config);
        return response.data;
    } catch (error) {
        throw new Error(`An error occurred: ${error as string}`);
    }
...
export const getById = async <T>(id: number, config: HeaderConfig) => {
    try {
        const response: AxiosResponse<T> = await axios.get(`${API.USER}/${id}`, config);
        return response.data;
    } catch (error) {
        throw new Error(`An error occurred: ${error as string}`);
    }
