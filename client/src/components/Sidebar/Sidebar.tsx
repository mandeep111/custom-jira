...
import { FormNewProject, FormNewSpace, DuplicateProject } from '../Form';
...
const Component = () => {
    ...
    const [dialogNewSpaceOpen, setDialogNewSpaceOpen] = React.useState<boolean>(false);
    const [dialogNewProjectOpen, setDialogNewProjectOpen] = React.useState<boolean>(false);
    const [sapceRename, setSpaceRename] = React.useState<boolean>(false);
    ...
    const spaceContextMenu: ContextMenu[] = [
        ...
        { title: 'Rename', fnc: () => setSpaceRename(true), icon: () => <PencilIcon className="icon-x16" />, class: 'text-default', child: [], break: false },
    ...
    const listContextMenu: ContextMenu[] = [
        ...
        { title: 'Copy link', fnc: undefined, icon: () => <LinkIcon className="icon-x16" />, class: 'text-default', child: [], break: false },
        { title: 'Duplicate', fnc: () => { setDuplicate(true); }, icon: () => <Square2StackIcon className="icon-x16" />, class: 'text-default', child: [], break: true },
    ...
    React.useEffect(() => {
        token ? void fetchSpaceList() : console.log(token);
    }, [token]);
    ...
    return (
        <React.Fragment>
                ...
                <nav>
                    ...
                    <Link to="/" className="flex items-center w-full">
                        <div className="flex mb-2">
                            <Squares2X2Icon className="icon-x24" />
                            <span className="text-default font-bold text-xxl mr-2">{'Everything'}</span>                 
                        </div>
                    </Link>
                                                ...
                                                <React.Fragment>
                                                    ...
                                                    >
                                                        ...
                                                        <Link to={data.id ? `/${data.id}/${data.url}` : ''} className="w-full">
                                                            ...
                                                            {sapceRename && data.id === spaceId ? (
                                                                <input
                                                                    type="text"
                                                                    className="text-default bg-default border border-default outline-none rounded-lg block w-full pl-10 p-2.5"
                                                                    placeholder="Search"
                                                                    onBlur={(event) => {
                                                                        event.target.value.length <= 0 ? setSpaceRename(false) : '';
                                                                    }}
                                                                />
                                                            ) : (
                                                                    ...
                                                                    <React.Fragment key={index}>
                                                                        ...
                                                                        >
                                                                            ...
                                                                            {isHoveredProject === index ? (
                                                                                <Menu as="div" className="relative inline-block text-left ml-auto z-50">
                                                                                    <div>
                                                                                        <Menu.Button className="flex items-center" onClick={() => setProjectId(project.id)}>
            ...
            {/* Create Duplicate */}
            <DuplicateProject isOpen={duplicate} setIsOpen={setDuplicate} fetchingData={fetchSpaceList} spaceId={spaceId} projectId={projectId} />
