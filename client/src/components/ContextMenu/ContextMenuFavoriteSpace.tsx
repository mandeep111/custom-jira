...
const Component = ({ favoriteSpaceRef, fetchMySpaceList, fetchFavSpaceList }: Props) => {
    ...
    const spaceContextMenu: ContextMenu[] = [
        {
            title: 'Create new', fnc: () => contextCreateRef.current!.click(), icon: () => <HeroIcons.PlusIcon className="icon-x16" />, class: 'text-default', child: [
                { title: 'Project', fnc: () => dispatch(setOpenFormNewProject(true)), icon: () => <HeroIcons.ListBulletIcon className="icon-x16" />, class: 'text-default', child: [], break: false },
        ...
        { title: 'Unfavorite', fnc: () => void handleRemoveFavorite(), icon: () => <StarIcon className="icon-x16 text-yellow-400" />, class: 'text-default', child: [], break: true },
        { title: 'Delete', fnc: () => void handleDelete(), icon: () => <HeroIcons.TrashIcon className="icon-x16" />, class: 'text-red-500', child: [], break: false },
    ];
    ...
    const handleRemoveFavorite = async () => {
        try {
            await Http.remove(`${API.SPACE}/favorite/${spaceId!}`);
        ...
        <React.Fragment>
            <div className="text-right">
                <Menu as="span" className="relative inline-block text-left ml-auto">
                    <Menu.Button ref={favoriteSpaceRef} className="flex items-center" />
                    <Transition
                        as={React.Fragment}
